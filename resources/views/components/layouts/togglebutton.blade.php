<div>
    <div class="flex mt-2" id="toggle-button">
        <div class="cursor-pointer flex flex-row items-center justify-center">
            <span class="text-xs text-gray-800"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4 dark:stroke-yellow-200">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
                </svg>
            </span>
            <div>
                <input type="checkbox" name="toggle" id="toggle" class="hidden">
                <label for="toggle">
                    <div
                        class="w-9 h-5 flex items-center bg-gray-300 dark:bg-cyan-700 rounded-full p-1 transform transition-colors ease-linear duration-150">
                        <div id="switch"
                            class="toggle-active transition-all w-4 h-4 bg-white rounded-full shadow-sm shadow-cyan-700 ease-linear duration-150">
                        </div>
                    </div>
                </label>
            </div>
            <span class="text-xs  text-black dark:text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" fill="none"
                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 dark:stroke-yellow-200">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                </svg>
            </span>
        </div>
    </div>

</div>
